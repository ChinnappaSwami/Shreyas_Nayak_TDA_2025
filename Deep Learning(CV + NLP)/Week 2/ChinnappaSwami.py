...
def find_ball(frame, movement_mask):
    ...
    for c in contours:
        ...
        if 0.75 <= circularity <= 1:  
            
            x = int(x)
            y = int(y)
            cv.circle(frame, (x, y), 10, (0, 255, 0), 2)
            return [x, y]
...
def identify_players(frame):
    yellow_team_count, red_team_count = 0 , 0 
    ...
    #red_lower = np.array([180, 135, 100])
    #red_upper = np.array([178, 255, 255])
    ...
    yellow_mask = cv.erode(yellow_mask, kernel, iterations=1)
    yellow_mask = cv.dilate(yellow_mask, kernel, iterations=1)
...
def capaing(video_path, output_path):
    ...
    while cap.isOpened():
        ...
        #cv.putText(frame, f"ball_position: {ball_position}", (90, 90), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
        ...
        cv.imshow('ChinnappaSwami"s Tracking', frame)
        out.write(frame)
...
output_video_path = 'inner_isshin.mp4'
